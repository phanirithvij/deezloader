...
class Deezer:
    def __init__(self, mail, password, token=None):
        ...
        post_data = {
            ...
            "https://www.deezer.com/ajax/action.php", post_data).text
        if "success" == end:
    ...
    def download(
        ...
        def ultimatum(infos, name, datas, quality, recursive_download, recursive_quality):
            ...
            if os.path.isfile(name):
                if not recursive_download:
                    return name
                ans = input(
                    "Track " + name + " already exists, do you want to redownload it?(y or n):")
                if ans != "y":
                    return name
            try:
            ...
            try:
                crypt = request(
                    "https://e-cdns-proxy-%s.dzcdn.net/mobile/1/%s" % (md5[0], hashs))
            except IndexError:
                raise TrackNotFound("Track not found :(")
        ...
        if "track" in link:
            ...
            image = request("https://e-cdns-images.dzcdn.net/images/cover/" +
                            infos['ALB_PICTURE'] + "/1200x1200-000000-80-0-0.jpg").content
            if len(image) == 13:
                image = request(
            ...
            try:
                ...
                url = request("https://api.deezer.com/search/track/?q=" + datas['music'].replace(
                    "#", "") + " + " + datas['artist'].replace("#", ""), True).json()
                try:
                    for b in range(url['total'] + 1):
                        if url['data'][b]['title'] == datas['music'] or\
                                datas['music'] in url['data'][b]['title_short']:
                            URL = url['data'][b]['link']
                            break
                except IndexError:
            ...
            image = request(
                ...
                infos[0]['ALB_PICTURE'] + "/1200x1200-000000-80-0-0.jpg"
            ).content
            if len(image) == 13:
                image = request(
            ...
            for a in tqdm(range(len(name))):
                ...
                try:
                    ...
                    url = request(
                        "https://api.deezer.com/search/track/?q=" + detas['music'].replace(
                            "#", "") + " + " + detas['artist'].replace("#", ""),
                        True
                    ).json()
                    try:
                        for b in range(url['total'] + 1):
                            if url['data'][b]['title'] == detas['music'] or\
                                    detas['music'] in url['data'][b]['title_short']:
                                URL = url['data'][b]['link']
                                break
                    except IndexError:
                        nams.append(name[a])
                        print("\nTrack not found: " +
                              detas['music'] + " - " + detas['artist'])
                        continue
                    try:
                        method = "song.getData"
                        json = {"sng_id": URL.split("/")[-1]}
                        nams.append(ultimatum(login(
                            method, json), name[a], detas, quality, recursive_download, recursive_quality))
                    except TrackNotFound:
                        nams.append(name[a])
                        print("\nTrack not found: " +
                              detas['music'] + " - " + detas['artist'])
    ...
    def download_trackdee(
        ...
        url1 = request("https://api.deezer.com/album/" +
                       str(url['album']['id']), True).json()
        ...
        try:
            ...
            pass
        name = dir / (album + " CD " +
                      datas['discnum'] + " TRACK " + datas['tracknum'])
        name = str(name.absolute())
        name, a = self.download(
            URL,
            name,
            quality,
            recursive_quality,
    ...
    def download_albumdee(
        ...
        for a in url['tracks']['data']:
            ...
            ur = request("https://api.deezer.com/track/" +
                         str(a['id']), True).json()
            discnum = str(ur['disk_number'])
            tracknum = str(ur['track_position'])
            names.append(
                str((dir / (album + " CD " + discnum + " TRACK " + tracknum)).absolute())
            )
    ...
    def download_playlistdee(self, URL, output=localdir, quality="MP3_320", recursive_quality=True, recursive_download=True):
        array = []
        if "?utm" in URL:
            URL, a = URL.split("?utm")
        url = request("https://api.deezer.com/playlist/" +
                      URL.split("/")[-1], True).json()
        for a in url['tracks']['data']:
            try:
                array.append(self.download_trackdee(
                    a['link'], output, quality, recursive_quality, recursive_download))
            except TrackNotFound:
                print("\nTrack not found " + a['title'])
                array.append("None")
        return array

    def download_trackspo(self, URL, output=localdir, quality="MP3_320", recursive_quality=True, recursive_download=True):
        if "?" in URL:
            URL, a = URL.split("?")
        try:
    ...
    def download_albumspo(
        ...
        try:
            ...
            try:
                url = request(
                    "https://api.deezer.com/track/isrc:" + isrc, True).json()
                names = self.download_albumdee(
                    url['album']['link'],
                    output,
                    quality,
    ...
    def download_playlistspo(self, URL, output=localdir, quality="MP3_320", recursive_quality=True, recursive_download=True):
        array = []
        if "?" in URL:
            URL, a = URL.split("?")
        URL = URL.split("/")
        ...
        for a in tracks['items']:
            try:
                array.append(self.download_trackspo(
                    a['track']['external_urls']['spotify'], output, quality, recursive_quality, recursive_download))
            except:
                print("\nTrack not found :(")
                array.append("None")
        if tracks['total'] != 100:
